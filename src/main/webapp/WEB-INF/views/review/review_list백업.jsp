            ...
            <!-- 파일 -->
           	 <c:forEach items="${listResult.mtr_review_file_list}" var="review_file_list"> 
	            	  ...
	            	  <c:if test="${review.mtrrv_seq eq review_file.mtrrv_seq}">
	                 		${review_file.mtrrvf_fname}
	          		  </c:if>
         		  </c:forEach>
            </c:forEach>
       </div>
        <div align="right">도움이됐어요 :<b>${review.mtrrv_likecnt}</b><img src="../resources/images/likeButton.png" alt="" width="30px" height="30px" id="likeBtn"/></div>
     </div><!-- 후기내용들까지 -->
